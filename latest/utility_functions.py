...
async def handle_events(message, websocket, sender_id):
    ...
    match message_type:
        ...
        case "request_join_session":
            ...
            client_to_add = {"player_id": sender_id,
                             "ready": True}
            active_sessions[session_id]["clients"].append(
                client_to_add)
            client_websocket_pairs[sender_id] = websocket
            await send_current_session_state(websocket, session_id)
        ...
        case "player_action":
            ...
            active_sessions[session_id]["action_list"].append(
                move_data)
            for client in active_sessions[session_id]["clients"]:
...
async def send_message_handler(message, client):
    message_json = await prepare_message_json(message_json)
    if client:  # Check if there are any clients connected
        await asyncio.gather(*(client.send(message_json) for client in connected_clients))


async def prepare_message_json(message):
    # prepare a JSON from message
    json_response = json.dumps(message)
    return json_response


async def send_to_everyone_in_session(message, session_id, skip_sender=False, sender_id=None):
    ...
    if session_id in active_sessions:
        for client in active_sessions[session_id]["clients"]:
            if skip_sender and client["player_id"] == sender_id:
                continue
            await send_message_handler(message, client_websocket_pairs[client["player_id"]])


async def send_to_client(message, client):
    await asyncio.gather(*(client.send(message) for client in connected_clients))
